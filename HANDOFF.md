...
**Last Updated:** 2026-06-22 — **LADDER LEAGUE BUILT + MERGED to `main` (4bf0ab9); CUDA bin built on cnc; run STAGED but DEFERRED on a production gate.** Iterated best-response vs the FROZEN tournament ladder (the population-based-training play justified by the 2026-06-21 tournament): warm-start from the 0.874 SOTA, train PFSP against frozen {sota,sp1term,sp1,gradclip,sp2}+7 archetypes (terminal reward), gate each candidate (winrate-vs-pool ≥ standing bar AND h2h-vs-CURRENT-SOTA ≥ 0.55), tournament-confirm #1 with rollback, promote validated winners into the frozen pool, stop after 2 no-improve rounds. Frozen opponents make the SP1/SP2 drift structurally impossible. Built via SDD (8 TDD tasks, all reviewed clean; T4 opus-reviewed; final whole-branch opus review caught + fixed 1 Important: gate was measuring h2h vs the ORIGINAL seed not the CURRENT SOTA → loosening bar each round). Full suite GREEN (66 pass). Additive (phase3/SP1/SP2 byte-unchanged). **▶ NEXT ACTION: launch the run on cnc on Matt's green-light** — `scripts/run_ladder_league_cnc.sh` (frees the 16GB P100 + EXIT-trap restore; bin already built `--features cuda`; contenders restored to cnc `bench/`). **⛔ NOT auto-launched:** openclaw `main` flagged a real production collision — the full-fleet-kill + multi-hour run (8 rounds × ~2h, stops at 2 no-improve) would straddle The Right Wire's MORNING `trw-voice` brief (no confirmed cloud fallback; voice-brief timer NOT locatable on cnc — only `trw-cnc-scrape` + nightdrive 07:35/08:30 visible). Stood down; fleet UP. Matt decides: launch whenever (confirm trw-voice timing / accept tradeoff) | after the morning brief | bounded (`--max-rounds 2`). Spec/plan: `docs/superpowers/{specs,plans}/2026-06-21-ladder-league*`. **Standing SOTA keeper unchanged: `bench/phase3-a1-combat/hac_best.safetensors` (0.871 ws / 0.874 decisive).**
