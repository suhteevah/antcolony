...
**Last Updated:** 2026-06-22 — **CROSS-SPECIES 1v1 ARENA ENGINE BUILT + MERGED to `main` (e8a5e8c)** + the full arena-expansion design suite (5 specs) + a citation-grounded biology dive. This is the pivot from "squeeze more recursive training out of the near-solved same-species arena" to **enriching the game so new strategy (and new competitive AIs) can emerge** — cross-species first because it breaks the symmetric-Nash that plateaued the same-species ladder, and it IS the on-roadmap science (Warren's *B. chinensis* displacement). **Built via SDD (8 TDD tasks, all reviewed; T3/T4/T5 opus-reviewed as load-bearing; opus final whole-branch review = merge-with-fixes, no hard blockers).** Per-colony species: `ColonySimConfig` + `colony_configs` on `Simulation`, `Species::apply_colony`, `new_two_colony_cross_species` (legacy ctor delegates **BYTE-IDENTICAL** — proven by a 300-tick bit-level guard); per-colony combat (venom×resistance clade matrix + terrain-gated `max_simultaneous_attackers` = Lanchester linear-law chokepoint lever); gated 2-phase interruptible queen-kill (`AntState::Usurping`, guarded on `usurp_gate_attacker_ratio > 0.0`); trainer `MatchEnv::new_cross_species`; determinism guard + BC-vs-AR smoke + `cross_species_matrix` win-matrix harness. Workspace builds, sim 225/0, trainer 68/0, det_check OK, additive audit clean. **▶ NEXT ACTIONS:** (1) **Balance-calibration phase** — run the win-matrix at mpe=50 (`scripts/run_cross_species_matrix.ps1`) + wire the now-inert biology knobs into species TOMLs: `usurp_corpse_to_killer_frac` (activates B7 predation / the displacement edge) and the usurp gate/channel values (activates B8 gated queen-kill) — calibrate against the matrix for intransitivity. (2) Decide the queen-melee-vs-gated-channel design Q (queens are still melee-killable; should cross-species require the channel?). (3) Document/persist the cross-species snapshot limitation (`from_snapshot_raw` drops per-colony species). (4) The other 4 arena specs are ready to build (N-colony FFA, nest layer, hazards/predators, map/terrain). Biology: `docs/biology/interspecific/*` (5 docs, ~180 verified sources, hallucinations caught). Specs: `docs/superpowers/specs/2026-06-22-arena-*`. Plan/ledger: `docs/superpowers/plans/2026-06-22-arena-cross-species.md` + `.superpowers/sdd/progress.md`.

(Prior 2026-06-22: **LADDER LEAGUE built + merged (`4bf0ab9`)** — iterated best-response vs the frozen tournament ladder (terminal reward, gate on h2h-vs-CURRENT-SOTA ≥ 0.55, tournament-confirm + rollback, stop after 2 no-improve). Built via SDD (8 tasks; opus final review fixed the gate-vs-original-SOTA bug). cnc CUDA bin built; an overnight run OOM'd (eval-on-GPU residue), fixed by moving eval to CPU (`bda34b6`); a round-1 probe then validated the OOM-fix but the **CPU eval is too slow** (`evaluate_h2h` sequential ~1h init) — the standing Ladder-League blocker = parallelize `evaluate_h2h` before another full run. The Ladder League run remains un-completed. Standing SOTA keeper: `bench/phase3-a1-combat/hac_best.safetensors` (0.871/0.874).)

(Prior 2026-06-21: **PvP tournament RAN on cnc** (~2.5h CPU, RAYON=3 to spare daytime inference). Ladder: **sota #1 (Elo 1522, .860)**, **sp1term dead-even #2 (1483, .829 — sota wins h2h only 0.517)**, aggressor #3, defender #4, sp1 #5, sp2 #6, ..., v1 #10 (.290), econ archetypes (economist/breeder/forager) LAST. **1 rock-paper-scissors cycle: heuristic▸sp2▸gradclip** (mid-pack; top is transitive). Read: SOTA genuinely best (ship-worthy) BUT self-play reached PEER + the 7-archetype bench over-weighted economy → it WAS a flawed target → population-based-training-vs-ladder is justified (= the Ladder League above). Results archived cnc `/opt/antcolony-archive/tournament-20260621/` + kokonoe `bench/tournament/`. Also this day: discovered cnc's Pascal candle fork (`/opt/candle-src`) doesn't build CPU-only — patched its dummy `CudaDevice` with a `supports_bf16_native(){false}` stub (safe; only compiled when cuda is OFF).)

(Prior 2026-06-20: PvP tournament yardstick built (`bin/tournament`, 4479d50). SP2 exploiter league built+merged (656d212), pilot NEGATIVE — 0 exploiter promotions, main degraded, 3rd self-play drift confirmation. Archived cnc `/opt/antcolony-archive/phase3-sp2/`.)
...
**◆ UPDATE (later 2026-06-24) — retrain ran, result is PARITY; root diagnosed; HAC route scoped:**
- Re-ran 2000 iters on the fixed trainer (`bench/xspecies-nest-venom3-fixed/`, ~30 min, avg_reward climbed 0.02→0.85). **But `eval_mlp_vs_heuristic` = 0.502 overall / 0.51 diagonal, FLAT across every snapshot (it250…it2000).** The trained flat brain is **dead-even with the heuristic** — no emergence.
...
Two arena expansions shipped to `main` this session (nest-layer + the cyclic clade type-chart). The headline result: **the cross-species win-matrix went from a strict transitive hierarchy (0 cycles) to a fully non-dominated rock-paper-scissors (60 three-cycles)** — but NOT via the nest layer. Key lesson: symmetric arena/defense mechanics change how long the *loser* survives, not *who wins*, so they preserve the stat hierarchy; intransitivity requires an **asymmetric matchup matrix**, which lives in combat traits (the venom/clade type-chart). Latest feature: `combat.venom_cycle_strength` + `clade::clade_cycle_multiplier` (commit `7ac95df`, opus review READY, full sim suite 213 green, byte-identical-off). Sweep (flat, mpe=50): strength 0→0 cycles, 2→24, 3→45 (formica_rufa's all-win broken), 5→60 (no all-win/all-lose). **OPEN (small):** pick the shipped default strength (rec 2–3, where stats+economy still matter) and bake it into the MatchEnv ctors / a config default — it's currently armed only via the `--venom-cycle <f32>` matrix flag.

**Prior this session — UNDERGROUND NEST-LAYER ARENA + RAID** (merged `38c43fe`, opus-reviewed merge-ready, byte-identical HOLDS): turns a deep-queen nest from an *unreachable bunker* into a *contested siege* — a small defender survives **8.3× longer** behind the tunnel chokepoint (flat wiped@188 vs nest 1555, 43 raiders descend). Champer & Schlenoff chokepoint. Grades the matrix (0→22 contested matchups) but — being symmetric — produces **0 cycles** (the negative result that motivated the venom-cycle lever). Also includes a species-differentiated body-size tunnel cap (`tunnel_cap_body_size_ref_mm`, commit `0b51ca5`) which proved inert on the matrix (tunnel combat doesn't decide realistic species matches).
...
**What Was Done:**
- **Ran the PvP tournament on cnc** (2026-06-21, ~2.5h CPU, RAYON=3 — main flagged daytime CPU-contention so we spared inference). Result = decisive "both": sota tops a real h2h ladder (Elo 1522) BUT sp1term (terminal-reward self-play brain) is dead-even #2 (1483; sota wins h2h only 0.517), econ archetypes rank LAST, 1 mid-pack cycle (heuristic▸sp2▸gradclip). Read: the 7-archetype bench was a flawed target; self-play is viable at peer → **population-based training against the ladder is the play.**
- **Built the Ladder League** (the above play) via brainstorm→spec→plan→SDD. 8 TDD tasks, all reviewed clean (T4 `train_round` opus-reviewed as load-bearing). Final whole-branch opus review caught 1 Important (gate measured h2h vs the ORIGINAL seed, not the CURRENT SOTA → loosens the bar each round) — FIXED (16dd816) + config validation + CLI exit codes. Full suite GREEN (66 pass, 0 fail). Fully additive. Merged ff to `main` (4bf0ab9).
- **Staged the cnc run:** shipped the code, built the bin `--features cuda` (7m38s, 38MB), restored the 5 frozen contenders to cnc `bench/`, hardened `run_ladder_league_cnc.sh` (frees the 16GB P100 + EXIT-trap restore).
- **Did NOT launch the run.** openclaw `main` flagged a production collision: full-fleet-kill + multi-hour run straddles The Right Wire's morning `trw-voice` brief, no confirmed cloud fallback, and the voice-brief timer isn't locatable on cnc (only `trw-cnc-scrape` + nightdrive 07:35/08:30). Stood down — fleet UP, production safe. Telegram'd Matt the decision.

**What's Next (Matt's call):**
1. **Launch the Ladder League run on cnc** — `scripts/run_ladder_league_cnc.sh` (NOTE: the ship doesn't copy `scripts/` to cnc — scp the hardened script or run its body inline; bin + contenders already in place). Options: launch whenever (confirm trw-voice timing / accept tradeoff) | after the morning brief | bounded `--max-rounds 2` to fit a window. Read `bench/ladder-league/` + the `LADDER_DONE` summary: did a trained brain EXCEED the 0.874 SOTA (tournament-confirmed #1, h2h ≥ 0.55)?
2. If it exceeds → new SOTA keeper; if it stops at 2 no-improve → honest "A1 + frozen-ladder objective at its ceiling" result (pivot to shipping SOTA into the game runtime).

**Notes for Next Session:**
- ⚠ **cnc overnight runs collide with The Right Wire `trw-voice` morning brief** — don't full-fleet-kill into the morning without confirming the brief's fire time + cloud fallback. See memory `feedback_cnc_overnight_trw_voice_gate`.
- ⚠ cnc Pascal candle fork (`/opt/candle-src`) needed a dummy-`CudaDevice` patch to build CPU-only (persists; re-breaks on a candle re-provision). See `project_cnc_provisioning_gotchas` Bonus 6.
- ⚠ The github-uploader added a benign "Initial commit" README touch (80077ef) mid-branch — harmless, carried into the merge.
- Ladder League spec/plan: `docs/superpowers/{specs,plans}/2026-06-21-ladder-league*`. SDD ledger: `.superpowers/sdd/progress.md`.
