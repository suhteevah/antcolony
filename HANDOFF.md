...
### Honest re-measure (mpe=50) — VALIDATED the 0.857; it firmed to 0.871
Re-scored the combat keeper on the eval.rs metric at **mpe=50** (the mpe=5 training evals are noisy) via a new CPU-only bin **`eval_winrate <ckpt> <mpe>`** (+ `scripts/run_eval_cnc.sh`; no GPU/fleet impact, ~70min on ~1 cnc core — the eval loop is single-threaded). Honest per-archetype: economist/breeder/forager **1.00**, conservative 0.88, heuristic 0.86, defender 0.72, **aggressor 0.64** (the mpe=5 0.40 was unlucky noise) → **MEAN 0.871** (the 0.857 firmed UP de-noised). vs v1's ~0.517 on the SAME eval.rs metric (both left-side) = a decisive **+0.35**.

### What's next (priority)
1. **`bench/phase3-a1-combat/hac_best.safetensors` (validated 0.871) is the SOTA — ship/distill it.** Remaining honest-comparison gap: still the eval.rs worker-share-at-timeout metric, NOT the harder `matchup_bench` DECISIVE-win metric (where v1 = 50.7%). The HAC can't run in matchup_bench directly (trainer-side net, not an `mlp:` brain spec) — bridge it (export HAC→runnable brain spec, or teach matchup_bench to load a HAC) for the final apples-to-apples. Until then, 0.871-vs-0.517 on eval.rs is the honest headline.
