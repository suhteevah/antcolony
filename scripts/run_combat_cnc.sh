#!/usr/bin/env bash
# Phase-3 A1 combat-reward run on cnc's 16GB P100 (UUID 17bd0d20).
...
echo "=== run start $(date -Is) reward=combat.toml out=bench/phase3-a1-combat ==="
# Same A1 config as the 0.629 grad-clip keeper; ONLY the reward changes
# (combat.toml = r6 + combat_loss_penalty=0.05) so the comparison isolates the
# combat-shaping lever's effect on the defender/aggressor axis.
./target/release/phase3_train \
  ...
  --reward assets/reward/combat.toml --out bench/phase3-a1-combat
code=$?
echo "=== run done $(date -Is) exit=$code ==="
echo "$code" > /opt/antcolony-cuda/run_combat.done
exit $code
