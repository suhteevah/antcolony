...
fn camponotus_is_tandem_recruiter_and_polymorphic_with_wood_substrate() {
    ...
    assert!(
        camp.colony_structure.polydomous,
        "Camponotus is polydomous (satellite nests)"
    );
    assert!(
        camp.substrate
            .preferred
            .iter()
            .any(|s| matches!(s, antcolony_sim::SubstrateType::Wood))
    );
}
...
fn formica_is_polygyne_supercolony_with_thatch_and_ranged_acid() {
    ...
    let f = species
        .iter()
        .find(|s| s.id == "formica_rufa")
        .expect("formica not loaded");
    ...
    assert!(
        f.diet_extended
            .host_species_required
            .iter()
            .any(|h| h == "formica_fusca")
    );
...
fn pogonomyrmex_is_granivore_with_sting_and_crater_disc() {
    ...
    let p = species
        .iter()
        .find(|s| s.id == "pogonomyrmex_occidentalis")
        .expect("pogo not loaded");
    ...
    assert_eq!(
        p.substrate.mound_construction,
        MoundConstruction::CraterDisc
    );
    assert!(
        !p.diet_extended.seed_dispersal,
        "Pogo eats seeds (granivory) ≠ disperses them (myrmecochory)"
    );
...
fn aphaenogaster_is_seed_disperser_with_keystone_role() {
    ...
    let a = species
        .iter()
        .find(|s| s.id == "aphaenogaster_rudis")
        .expect("aphae not loaded");
    assert!(
        a.diet_extended.seed_dispersal,
        "Aphaenogaster IS the myrmecochore"
    );
...
fn tapinoma_is_polydomous_supercolony_with_chemical_alarm() {
    ...
    let t = species
        .iter()
        .find(|s| s.id == "tapinoma_sessile")
        .expect("tapinoma not loaded");
    ...
    assert!(
        t.colony_structure.relocation_tendency > 0.0,
        "Tapinoma should have non-zero relocation tendency"
    );
